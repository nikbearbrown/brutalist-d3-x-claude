# Prompt — Chapter Enrichment: Tables, Figures, and Wayback Machine

overwrite any existing graphics except peoples names. (i.e. Wayback machine)

Only add a Wayback machine if it does not exist, otherwise skip it
...
- Updates every `## AI Wayback Machine` section: replaces any subject born after 2000 (or lacking strong historical significance) with an appropriate pre-2000 figure, and inserts a properly-formatted AI-generated portrait stub
...
### PASS 3 — AI Wayback Machine sections

Scan each chapter for:
...
> No person who was alive and producing significant work after 2000 should appear in an AI Wayback Machine section.

If the subject fails, replace with a historically appropriate figure:
- Died before 2001, OR foundational work entirely pre-2001
- Genuinely connected to the chapter's intellectual substance
- Not already used in another chapter's Wayback Machine section

**Step 2 — Insert the portrait stub** immediately after the opening paragraph and before the "Run this:" block:

```markdown
![{Person's full name}, {approximate date or era}. AI-generated {portrait/illustration} based on a public domain {photograph/engraving/painting}.](../images/{filename}.jpg)
*{Person's full name}, {approximate date or era}. AI-generated {type} based on a public domain {source type} (Wikimedia Commons).*
```

Rules:
- Post-1850s: source type = `photograph`, type = `portrait`
- Pre-1850s: source type = `engraving` or `painting`, type = `portrait` or `illustration`
- Filename: name lowercased, hyphenated, `.jpg`
- Alt text and caption say the same thing. Both required. No real Wikimedia URLs.
- If a correctly-formatted stub already exists, leave it.

**Step 3 — Update the run-this prompt** if the subject changed — rename consistently throughout the section.

---

### PASS 4 — Write back and report
...
```
{filename} — {N} tables rendered, {N} SVGs generated, {N} D3 HTML files generated, Wayback Machine: {kept/replaced (new subject name)}
```
...
Total Wayback Machine subjects replaced: {N}
```
