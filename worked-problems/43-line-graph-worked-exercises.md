# Worked Exercises: Line Graph

*Chapter 43 of Brutalist d3 x Claude*
...
- Line Graph — know the chapter's central visualization task.
...
**What this demonstrates:** This example makes visible how the design logic of Line Graph connects data structure, reader task, marks, channels, and implementation constraints.

**The problem:**

Evaluate this chapter-grounded visualization case: A line graph encodes quantitative values as position along the y-axis — the most accurate perceptual channel in Cleveland and McGill's hierarchy — and connects successive points with a line that implies continuity. The line itself is not a data encoding; it is a visual metaphor for the path a value took between measurements. An upward slope means increase; a downward slope means decrease; diverging lines mean a growing gap. These readings are pre-attentive — the direction is understood before any axis is read. That metaphor only works when the x-axis is genuinely continuous (time, distance, sequence), never when it is categorical.
...
**Final answer:** The correct solution is to treat Line Graph as a design response to a specific reader task and data structure, then express that decision through explicit marks, channels, and D3 implementation constraints.

**What made this work:** The solution worked because it did not stop at naming a chart type. It connected the reader task to the data structure, mapped attributes onto visual channels, rejected a misleading alternative, and translated the result into implementation language. A naive approach would ask Claude to "make a Line Graph" and hope the defaults matched the chapter's design logic.
...
A new dataset has the same underlying visualization problem as Line Graph, but the labels and values are different. Decide which marks, channels, and reader task should govern the design.
...
A Claude Code prompt for a Line Graph is missing the encoding rationale and the quality check. Complete the missing reasoning steps before code generation.

**Partial solution:**

**Step 1 — Name the reader task**
The reader must perform the task that Line Graph is built to support, not merely recognize the chart form.
...
**Final answer:** The completed reasoning should justify Line Graph through the connection between reader task, data structure, encoding choice, and implementation constraints.
...
A student proposes a Line Graph for data that superficially resembles the chapter example. The solution sounds plausible, but one design move violates the chapter's encoding logic.

**The flawed solution:**

**Step 1 — Name the chart type**
The student identifies Line Graph as the intended form.
...
Use the principle from Line Graph in a different domain: civic data, classroom analytics, public health, finance, or operations. The surface topic changes; the reader task and encoding logic stay central.
...
**Problem F1:**
A dataset appears suitable for Line Graph. Decide what marks and channels would make the reader task explicit, and name one implementation requirement for the D3 version.
*Chapter this draws from: Chapter 43 — Line Graph*

**Problem F2:**
A second dataset looks more like Kagi Chart. Decide whether the previous chapter's method or the present chapter's method better matches the reader task.
*Chapter this draws from: Chapter 42 — Kagi Chart*

**Problem F3:**
A polished chart appears to use the right visual family, but the decisive issue is whether the encoded data structure matches Line Graph or a different chart logic from an earlier chapter. Choose the governing concept and justify it before solving.
