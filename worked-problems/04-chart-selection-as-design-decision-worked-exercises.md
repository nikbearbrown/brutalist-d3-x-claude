# Worked Exercises: Chart Selection as Design Decision

*Chapter 04 of Brutalist d3 x Claude*
...
- Chart Selection as Design Decision — know the chapter's central visualization task.
- mark — understand how this term functions in the chapter, not just its definition.
- Why the Pie Chart Was Chosen Anyway — be able to explain why this section shapes the design decision.

---

## Part A — Full Worked Example

**What this demonstrates:** This example makes visible how the design logic of Chart Selection as Design Decision connects data structure, reader task, marks, channels, and implementation constraints.

**The problem:**

Evaluate this chapter-grounded visualization case: Here is a chart that was published in a humanitarian report by a competent, experienced organization. It had a title: "Allocation of Emergency Response Funds, FY2021." It had fourteen slices. Each slice represented one funding category — food security, water and sanitation, shelter, health, protection, education, livelihoods, logistics, coordination, security, communications, transport, monitoring, and a residual "other." The slices ranged from 21% down to under 1%.
...
Use the chapter vocabulary mark, channel, and encoding to specify which visual marks carry which data attributes.
...
**Final answer:** The correct solution is to treat Chart Selection as Design Decision as a design response to a specific reader task and data structure, then express that decision through explicit marks, channels, and D3 implementation constraints.

**What made this work:** The solution worked because it did not stop at naming a chart type. It connected the reader task to the data structure, mapped attributes onto visual channels, rejected a misleading alternative, and translated the result into implementation language. A naive approach would ask Claude to "make a Chart Selection as Design Decision" and hope the defaults matched the chapter's design logic.
...
A new dataset has the same underlying visualization problem as Chart Selection as Design Decision, but the labels and values are different. Decide which marks, channels, and reader task should govern the design.
...
A Claude Code prompt for a Chart Selection as Design Decision is missing the encoding rationale and the quality check. Complete the missing reasoning steps before code generation.

**Partial solution:**

**Step 1 — Name the reader task**
The reader must perform the task that Chart Selection as Design Decision is built to support, not merely recognize the chart form.
...
**Final answer:** The completed reasoning should justify Chart Selection as Design Decision through the connection between reader task, data structure, encoding choice, and implementation constraints.
...
A student proposes a Chart Selection as Design Decision for data that superficially resembles the chapter example. The solution sounds plausible, but one design move violates the chapter's encoding logic.

**The flawed solution:**

**Step 1 — Name the chart type**
The student identifies Chart Selection as Design Decision as the intended form.
...
Use the principle from Chart Selection as Design Decision in a different domain: civic data, classroom analytics, public health, finance, or operations. The surface topic changes; the reader task and encoding logic stay central.
...
**Problem F1:**
A dataset appears suitable for Chart Selection as Design Decision. Decide what marks and channels would make the reader task explicit, and name one implementation requirement for the D3 version.
*Chapter this draws from: Chapter 04 — Chart Selection as Design Decision*

**Problem F2:**
A second dataset looks more like Marks and Channels. Decide whether the previous chapter's method or the present chapter's method better matches the reader task.
*Chapter this draws from: Chapter 03 — Marks and Channels*

**Problem F3:**
A polished chart appears to use the right visual family, but the decisive issue is whether the encoded data structure matches Chart Selection as Design Decision or a different chart logic from an earlier chapter. Choose the governing concept and justify it before solving.
...
- The student cannot explain how Why the Pie Chart Was Chosen Anyway connects to the chart choice.
