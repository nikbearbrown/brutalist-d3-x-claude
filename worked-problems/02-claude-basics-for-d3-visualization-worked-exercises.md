# Worked Exercises: Claude Basics for D3 Visualization

*Chapter 02 of Brutalist d3 x Claude*
...
- Claude Basics for D3 Visualization — know the chapter's central visualization task.
- marks — understand how this term functions in the chapter, not just its definition.
- Where Claude lives — be able to explain why this section shapes the design decision.

---

## Part A — Full Worked Example

**What this demonstrates:** This example makes visible how the design logic of Claude Basics for D3 Visualization connects data structure, reader task, marks, channels, and implementation constraints.

**The problem:**

Evaluate this chapter-grounded visualization case: The gap between "make a chart" and "make the chart" is the whole problem.
...
Use the chapter vocabulary marks, channel, and channels to specify which visual marks carry which data attributes.
...
**Final answer:** The correct solution is to treat Claude Basics for D3 Visualization as a design response to a specific reader task and data structure, then express that decision through explicit marks, channels, and D3 implementation constraints.

**What made this work:** The solution worked because it did not stop at naming a chart type. It connected the reader task to the data structure, mapped attributes onto visual channels, rejected a misleading alternative, and translated the result into implementation language. A naive approach would ask Claude to "make a Claude Basics for D3 Visualization" and hope the defaults matched the chapter's design logic.
...
A new dataset has the same underlying visualization problem as Claude Basics for D3 Visualization, but the labels and values are different. Decide which marks, channels, and reader task should govern the design.
...
A Claude Code prompt for a Claude Basics for D3 Visualization is missing the encoding rationale and the quality check. Complete the missing reasoning steps before code generation.

**Partial solution:**

**Step 1 — Name the reader task**
The reader must perform the task that Claude Basics for D3 Visualization is built to support, not merely recognize the chart form.
...
**Final answer:** The completed reasoning should justify Claude Basics for D3 Visualization through the connection between reader task, data structure, encoding choice, and implementation constraints.
...
A student proposes a Claude Basics for D3 Visualization for data that superficially resembles the chapter example. The solution sounds plausible, but one design move violates the chapter's encoding logic.

**The flawed solution:**

**Step 1 — Name the chart type**
The student identifies Claude Basics for D3 Visualization as the intended form.
...
Use the principle from Claude Basics for D3 Visualization in a different domain: civic data, classroom analytics, public health, finance, or operations. The surface topic changes; the reader task and encoding logic stay central.
...
**Problem F1:**
A dataset appears suitable for Claude Basics for D3 Visualization. Decide what marks and channels would make the reader task explicit, and name one implementation requirement for the D3 version.
*Chapter this draws from: Chapter 02 — Claude Basics for D3 Visualization*

**Problem F2:**
A second dataset looks more like Introduction. Decide whether the previous chapter's method or the present chapter's method better matches the reader task.
*Chapter this draws from: Chapter 00 — Introduction*

**Problem F3:**
A polished chart appears to use the right visual family, but the decisive issue is whether the encoded data structure matches Claude Basics for D3 Visualization or a different chart logic from an earlier chapter. Choose the governing concept and justify it before solving.
...
- The student cannot explain how Where Claude lives connects to the chart choice.
