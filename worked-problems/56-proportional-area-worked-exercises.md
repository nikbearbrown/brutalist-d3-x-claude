# Worked Exercises: Proportional Area

*Chapter 56 of Brutalist d3 x Claude*
...
- Proportional Area — know the chapter's central visualization task.
...
**What this demonstrates:** This example makes visible how the design logic of Proportional Area connects data structure, reader task, marks, channels, and implementation constraints.

**The problem:**

Evaluate this chapter-grounded visualization case: A Proportional Area Chart encodes quantitative values as the area of geometric shapes — most commonly circles, but any closed shape works. The perceptual mechanism is area estimation : the eye reads the space inside the shape and compares it across items. Area estimation is less precise than position on a common axis (the bar chart's mechanism), which makes proportional area charts better suited to presentational contexts — conveying the scale of differences at a glance — than to analytical ones, where readers need to extract specific values. Value labels inside or beside circles are required; the chart alone cannot support accurate reading.
...
**Final answer:** The correct solution is to treat Proportional Area as a design response to a specific reader task and data structure, then express that decision through explicit marks, channels, and D3 implementation constraints.

**What made this work:** The solution worked because it did not stop at naming a chart type. It connected the reader task to the data structure, mapped attributes onto visual channels, rejected a misleading alternative, and translated the result into implementation language. A naive approach would ask Claude to "make a Proportional Area" and hope the defaults matched the chapter's design logic.
...
A new dataset has the same underlying visualization problem as Proportional Area, but the labels and values are different. Decide which marks, channels, and reader task should govern the design.
...
A Claude Code prompt for a Proportional Area is missing the encoding rationale and the quality check. Complete the missing reasoning steps before code generation.

**Partial solution:**

**Step 1 — Name the reader task**
The reader must perform the task that Proportional Area is built to support, not merely recognize the chart form.
...
**Final answer:** The completed reasoning should justify Proportional Area through the connection between reader task, data structure, encoding choice, and implementation constraints.
...
A student proposes a Proportional Area for data that superficially resembles the chapter example. The solution sounds plausible, but one design move violates the chapter's encoding logic.

**The flawed solution:**

**Step 1 — Name the chart type**
The student identifies Proportional Area as the intended form.
...
Use the principle from Proportional Area in a different domain: civic data, classroom analytics, public health, finance, or operations. The surface topic changes; the reader task and encoding logic stay central.
...
**Problem F1:**
A dataset appears suitable for Proportional Area. Decide what marks and channels would make the reader task explicit, and name one implementation requirement for the D3 version.
*Chapter this draws from: Chapter 56 — Proportional Area*

**Problem F2:**
A second dataset looks more like Population Pyramid. Decide whether the previous chapter's method or the present chapter's method better matches the reader task.
*Chapter this draws from: Chapter 55 — Population Pyramid*

**Problem F3:**
A polished chart appears to use the right visual family, but the decisive issue is whether the encoded data structure matches Proportional Area or a different chart logic from an earlier chapter. Choose the governing concept and justify it before solving.
