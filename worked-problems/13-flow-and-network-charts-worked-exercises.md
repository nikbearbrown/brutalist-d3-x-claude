# Worked Exercises: Flow and Network Charts

*Chapter 13 of Brutalist d3 x Claude*
...
- Flow and Network Charts — know the chapter's central visualization task.
- channel — understand how this term functions in the chapter, not just its definition.
- Width as a channel — be able to explain why this section shapes the design decision.

---

## Part A — Full Worked Example

**What this demonstrates:** This example makes visible how the design logic of Flow and Network Charts connects data structure, reader task, marks, channels, and implementation constraints.

**The problem:**

Evaluate this chapter-grounded visualization case: Now suppose someone made the same diagram with every band the same width. It would still show the paths — oil connects to transportation fuel, coal connects to electricity. But you would learn nothing about magnitude. The chart would have become a topology diagram: does this connection exist? The original chart answers a different question: how much flows along it?
...
**Final answer:** The correct solution is to treat Flow and Network Charts as a design response to a specific reader task and data structure, then express that decision through explicit marks, channels, and D3 implementation constraints.

**What made this work:** The solution worked because it did not stop at naming a chart type. It connected the reader task to the data structure, mapped attributes onto visual channels, rejected a misleading alternative, and translated the result into implementation language. A naive approach would ask Claude to "make a Flow and Network Charts" and hope the defaults matched the chapter's design logic.
...
A new dataset has the same underlying visualization problem as Flow and Network Charts, but the labels and values are different. Decide which marks, channels, and reader task should govern the design.
...
A Claude Code prompt for a Flow and Network Charts is missing the encoding rationale and the quality check. Complete the missing reasoning steps before code generation.

**Partial solution:**

**Step 1 — Name the reader task**
The reader must perform the task that Flow and Network Charts is built to support, not merely recognize the chart form.
...
**Final answer:** The completed reasoning should justify Flow and Network Charts through the connection between reader task, data structure, encoding choice, and implementation constraints.
...
A student proposes a Flow and Network Charts for data that superficially resembles the chapter example. The solution sounds plausible, but one design move violates the chapter's encoding logic.

**The flawed solution:**

**Step 1 — Name the chart type**
The student identifies Flow and Network Charts as the intended form.
...
Use the principle from Flow and Network Charts in a different domain: civic data, classroom analytics, public health, finance, or operations. The surface topic changes; the reader task and encoding logic stay central.
...
**Problem F1:**
A dataset appears suitable for Flow and Network Charts. Decide what marks and channels would make the reader task explicit, and name one implementation requirement for the D3 version.
*Chapter this draws from: Chapter 13 — Flow and Network Charts*

**Problem F2:**
A second dataset looks more like Hierarchy Charts. Decide whether the previous chapter's method or the present chapter's method better matches the reader task.
*Chapter this draws from: Chapter 12 — Hierarchy Charts*

**Problem F3:**
A polished chart appears to use the right visual family, but the decisive issue is whether the encoded data structure matches Flow and Network Charts or a different chart logic from an earlier chapter. Choose the governing concept and justify it before solving.
...
- The student cannot explain how Width as a channel connects to the chart choice.
