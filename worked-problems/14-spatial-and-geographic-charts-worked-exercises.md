# Worked Exercises: Spatial and Geographic Charts

*Chapter 14 of Brutalist d3 x Claude*
...
- Spatial and Geographic Charts — know the chapter's central visualization task.
- mark — understand how this term functions in the chapter, not just its definition.
- What Geographic Visualization Is For — be able to explain why this section shapes the design decision.

---

## Part A — Full Worked Example

**What this demonstrates:** This example makes visible how the design logic of Spatial and Geographic Charts connects data structure, reader task, marks, channels, and implementation constraints.

**The problem:**

Evaluate this chapter-grounded visualization case: Now here is the same map shaded by the uninsured rate — the percentage of residents without health insurance. Texas is still dark. But California is now much lighter. Wyoming, which looked pale before, is now darker than California. Rhode Island, invisible in the first map, turns out to have a lower rate than the national average. The entire visual story has changed.
...
**Final answer:** The correct solution is to treat Spatial and Geographic Charts as a design response to a specific reader task and data structure, then express that decision through explicit marks, channels, and D3 implementation constraints.

**What made this work:** The solution worked because it did not stop at naming a chart type. It connected the reader task to the data structure, mapped attributes onto visual channels, rejected a misleading alternative, and translated the result into implementation language. A naive approach would ask Claude to "make a Spatial and Geographic Charts" and hope the defaults matched the chapter's design logic.
...
A new dataset has the same underlying visualization problem as Spatial and Geographic Charts, but the labels and values are different. Decide which marks, channels, and reader task should govern the design.
...
A Claude Code prompt for a Spatial and Geographic Charts is missing the encoding rationale and the quality check. Complete the missing reasoning steps before code generation.

**Partial solution:**

**Step 1 — Name the reader task**
The reader must perform the task that Spatial and Geographic Charts is built to support, not merely recognize the chart form.
...
**Final answer:** The completed reasoning should justify Spatial and Geographic Charts through the connection between reader task, data structure, encoding choice, and implementation constraints.
...
A student proposes a Spatial and Geographic Charts for data that superficially resembles the chapter example. The solution sounds plausible, but one design move violates the chapter's encoding logic.

**The flawed solution:**

**Step 1 — Name the chart type**
The student identifies Spatial and Geographic Charts as the intended form.
...
Use the principle from Spatial and Geographic Charts in a different domain: civic data, classroom analytics, public health, finance, or operations. The surface topic changes; the reader task and encoding logic stay central.
...
**Problem F1:**
A dataset appears suitable for Spatial and Geographic Charts. Decide what marks and channels would make the reader task explicit, and name one implementation requirement for the D3 version.
*Chapter this draws from: Chapter 14 — Spatial and Geographic Charts*

**Problem F2:**
A second dataset looks more like Flow and Network Charts. Decide whether the previous chapter's method or the present chapter's method better matches the reader task.
*Chapter this draws from: Chapter 13 — Flow and Network Charts*

**Problem F3:**
A polished chart appears to use the right visual family, but the decisive issue is whether the encoded data structure matches Spatial and Geographic Charts or a different chart logic from an earlier chapter. Choose the governing concept and justify it before solving.
...
- The student cannot explain how What Geographic Visualization Is For connects to the chart choice.
