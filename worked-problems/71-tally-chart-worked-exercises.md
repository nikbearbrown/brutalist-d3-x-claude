# Worked Exercises: Tally Chart

*Chapter 71 of Brutalist d3 x Claude*
...
- Tally Chart — know the chapter's central visualization task.
...
**What this demonstrates:** This example makes visible how the design logic of Tally Chart connects data structure, reader task, marks, channels, and implementation constraints.

**The problem:**

Evaluate this chapter-grounded visualization case: A Tally Chart is simultaneously a data collection method and a visual display . The tally mark numeral system — four vertical strokes crossed by a diagonal — groups counts in fives, making totals readable at a glance without arithmetic. Historically made with pen and paper, the form conveys something that no bar chart can: the process of counting . Each mark is a discrete event. The chart does not abstract the data into a continuous scale; it preserves the unit nature of what was counted. The animation here restores that accumulative process — marks appear as they would be drawn, one incident at a time.
...
**Final answer:** The correct solution is to treat Tally Chart as a design response to a specific reader task and data structure, then express that decision through explicit marks, channels, and D3 implementation constraints.

**What made this work:** The solution worked because it did not stop at naming a chart type. It connected the reader task to the data structure, mapped attributes onto visual channels, rejected a misleading alternative, and translated the result into implementation language. A naive approach would ask Claude to "make a Tally Chart" and hope the defaults matched the chapter's design logic.
...
A new dataset has the same underlying visualization problem as Tally Chart, but the labels and values are different. Decide which marks, channels, and reader task should govern the design.
...
A Claude Code prompt for a Tally Chart is missing the encoding rationale and the quality check. Complete the missing reasoning steps before code generation.

**Partial solution:**

**Step 1 — Name the reader task**
The reader must perform the task that Tally Chart is built to support, not merely recognize the chart form.
...
**Final answer:** The completed reasoning should justify Tally Chart through the connection between reader task, data structure, encoding choice, and implementation constraints.
...
A student proposes a Tally Chart for data that superficially resembles the chapter example. The solution sounds plausible, but one design move violates the chapter's encoding logic.

**The flawed solution:**

**Step 1 — Name the chart type**
The student identifies Tally Chart as the intended form.
...
Use the principle from Tally Chart in a different domain: civic data, classroom analytics, public health, finance, or operations. The surface topic changes; the reader task and encoding logic stay central.
...
**Problem F1:**
A dataset appears suitable for Tally Chart. Decide what marks and channels would make the reader task explicit, and name one implementation requirement for the D3 version.
*Chapter this draws from: Chapter 71 — Tally Chart*

**Problem F2:**
A second dataset looks more like Sunburst. Decide whether the previous chapter's method or the present chapter's method better matches the reader task.
*Chapter this draws from: Chapter 70 — Sunburst*

**Problem F3:**
A polished chart appears to use the right visual family, but the decisive issue is whether the encoded data structure matches Tally Chart or a different chart logic from an earlier chapter. Choose the governing concept and justify it before solving.
