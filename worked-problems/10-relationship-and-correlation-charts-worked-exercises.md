# Worked Exercises: Relationship and Correlation Charts

*Chapter 10 of Brutalist d3 x Claude*
...
- Relationship and Correlation Charts — know the chapter's central visualization task.
- mark — understand how this term functions in the chapter, not just its definition.
- What a Scatterplot Actually Is — be able to explain why this section shapes the design decision.

---

## Part A — Full Worked Example

**What this demonstrates:** This example makes visible how the design logic of Relationship and Correlation Charts connects data structure, reader task, marks, channels, and implementation constraints.

**The problem:**

Evaluate this chapter-grounded visualization case: Here is a chart. Each point is a country. The x-axis shows the education index — a composite score between 0 and 1 measuring school enrollment and years of schooling. The y-axis shows life expectancy in years. There are about 170 points. An OLS regression line runs through the cloud, sloped upward and to the right. Pearson's r is annotated near the line: r = 0.79.
...
**Final answer:** The correct solution is to treat Relationship and Correlation Charts as a design response to a specific reader task and data structure, then express that decision through explicit marks, channels, and D3 implementation constraints.

**What made this work:** The solution worked because it did not stop at naming a chart type. It connected the reader task to the data structure, mapped attributes onto visual channels, rejected a misleading alternative, and translated the result into implementation language. A naive approach would ask Claude to "make a Relationship and Correlation Charts" and hope the defaults matched the chapter's design logic.
...
A new dataset has the same underlying visualization problem as Relationship and Correlation Charts, but the labels and values are different. Decide which marks, channels, and reader task should govern the design.
...
A Claude Code prompt for a Relationship and Correlation Charts is missing the encoding rationale and the quality check. Complete the missing reasoning steps before code generation.

**Partial solution:**

**Step 1 — Name the reader task**
The reader must perform the task that Relationship and Correlation Charts is built to support, not merely recognize the chart form.
...
**Final answer:** The completed reasoning should justify Relationship and Correlation Charts through the connection between reader task, data structure, encoding choice, and implementation constraints.
...
A student proposes a Relationship and Correlation Charts for data that superficially resembles the chapter example. The solution sounds plausible, but one design move violates the chapter's encoding logic.

**The flawed solution:**

**Step 1 — Name the chart type**
The student identifies Relationship and Correlation Charts as the intended form.
...
Use the principle from Relationship and Correlation Charts in a different domain: civic data, classroom analytics, public health, finance, or operations. The surface topic changes; the reader task and encoding logic stay central.
...
**Problem F1:**
A dataset appears suitable for Relationship and Correlation Charts. Decide what marks and channels would make the reader task explicit, and name one implementation requirement for the D3 version.
*Chapter this draws from: Chapter 10 — Relationship and Correlation Charts*

**Problem F2:**
A second dataset looks more like Distribution Charts. Decide whether the previous chapter's method or the present chapter's method better matches the reader task.
*Chapter this draws from: Chapter 09 — Distribution Charts*

**Problem F3:**
A polished chart appears to use the right visual family, but the decisive issue is whether the encoded data structure matches Relationship and Correlation Charts or a different chart logic from an earlier chapter. Choose the governing concept and justify it before solving.
...
- The student cannot explain how What a Scatterplot Actually Is connects to the chart choice.
