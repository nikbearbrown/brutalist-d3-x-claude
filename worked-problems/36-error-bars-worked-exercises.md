# Worked Exercises: Error Bars

*Chapter 36 of Brutalist d3 x Claude*
...
- Error Bars — know the chapter's central visualization task.
- mark — understand how this term functions in the chapter, not just its definition.
- The perceptual mechanism — be able to explain why this section shapes the design decision.

---

## Part A — Full Worked Example

**What this demonstrates:** This example makes visible how the design logic of Error Bars connects data structure, reader task, marks, channels, and implementation constraints.

**The problem:**

Evaluate this chapter-grounded visualization case: Error bars are not a chart type — they are a graphical enhancement layered onto a Cartesian graph. They exploit the same channel as the primary encoding (position along a quantitative axis) to add a second layer of information: the spread or uncertainty around each plotted value. The cap-tipped line extending from a central point forces the eye to read a range rather than a single value, making it immediately visible whether groups have tight, reliable measurements or wide, unreliable ones.
...
**Final answer:** The correct solution is to treat Error Bars as a design response to a specific reader task and data structure, then express that decision through explicit marks, channels, and D3 implementation constraints.

**What made this work:** The solution worked because it did not stop at naming a chart type. It connected the reader task to the data structure, mapped attributes onto visual channels, rejected a misleading alternative, and translated the result into implementation language. A naive approach would ask Claude to "make a Error Bars" and hope the defaults matched the chapter's design logic.
...
A new dataset has the same underlying visualization problem as Error Bars, but the labels and values are different. Decide which marks, channels, and reader task should govern the design.
...
A Claude Code prompt for a Error Bars is missing the encoding rationale and the quality check. Complete the missing reasoning steps before code generation.

**Partial solution:**

**Step 1 — Name the reader task**
The reader must perform the task that Error Bars is built to support, not merely recognize the chart form.
...
**Final answer:** The completed reasoning should justify Error Bars through the connection between reader task, data structure, encoding choice, and implementation constraints.
...
A student proposes a Error Bars for data that superficially resembles the chapter example. The solution sounds plausible, but one design move violates the chapter's encoding logic.

**The flawed solution:**

**Step 1 — Name the chart type**
The student identifies Error Bars as the intended form.
...
Use the principle from Error Bars in a different domain: civic data, classroom analytics, public health, finance, or operations. The surface topic changes; the reader task and encoding logic stay central.
...
**Problem F1:**
A dataset appears suitable for Error Bars. Decide what marks and channels would make the reader task explicit, and name one implementation requirement for the D3 version.
*Chapter this draws from: Chapter 36 — Error Bars*

**Problem F2:**
A second dataset looks more like Dot Matrix. Decide whether the previous chapter's method or the present chapter's method better matches the reader task.
*Chapter this draws from: Chapter 35 — Dot Matrix*

**Problem F3:**
A polished chart appears to use the right visual family, but the decisive issue is whether the encoded data structure matches Error Bars or a different chart logic from an earlier chapter. Choose the governing concept and justify it before solving.
...
- The student cannot explain how The perceptual mechanism connects to the chart choice.
