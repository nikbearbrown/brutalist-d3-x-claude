# Worked Exercises: Parallel Coordinates

*Chapter 50 of Brutalist d3 x Claude*
...
- Parallel Coordinates — know the chapter's central visualization task.
- channel — understand how this term functions in the chapter, not just its definition.
- What this chart is — be able to explain why this section shapes the design decision.

---

## Part A — Full Worked Example

**What this demonstrates:** This example makes visible how the design logic of Parallel Coordinates connects data structure, reader task, marks, channels, and implementation constraints.

**The problem:**

Evaluate this chapter-grounded visualization case: A parallel coordinates plot represents each observation as a polyline that crosses multiple vertical axes. Each axis encodes one variable; the position where a line crosses an axis encodes that variable's value. The perceptual mechanism is pattern detection through line slope and clustering — lines that run parallel between two adjacent axes indicate positive correlation; lines that cross indicate negative correlation or inverse relationship.
...
Use the chapter vocabulary channel, encoding, and D3 to specify which visual marks carry which data attributes.
...
**Final answer:** The correct solution is to treat Parallel Coordinates as a design response to a specific reader task and data structure, then express that decision through explicit marks, channels, and D3 implementation constraints.

**What made this work:** The solution worked because it did not stop at naming a chart type. It connected the reader task to the data structure, mapped attributes onto visual channels, rejected a misleading alternative, and translated the result into implementation language. A naive approach would ask Claude to "make a Parallel Coordinates" and hope the defaults matched the chapter's design logic.
...
A new dataset has the same underlying visualization problem as Parallel Coordinates, but the labels and values are different. Decide which marks, channels, and reader task should govern the design.
...
A Claude Code prompt for a Parallel Coordinates is missing the encoding rationale and the quality check. Complete the missing reasoning steps before code generation.

**Partial solution:**

**Step 1 — Name the reader task**
The reader must perform the task that Parallel Coordinates is built to support, not merely recognize the chart form.
...
**Final answer:** The completed reasoning should justify Parallel Coordinates through the connection between reader task, data structure, encoding choice, and implementation constraints.
...
A student proposes a Parallel Coordinates for data that superficially resembles the chapter example. The solution sounds plausible, but one design move violates the chapter's encoding logic.

**The flawed solution:**

**Step 1 — Name the chart type**
The student identifies Parallel Coordinates as the intended form.
...
Use the principle from Parallel Coordinates in a different domain: civic data, classroom analytics, public health, finance, or operations. The surface topic changes; the reader task and encoding logic stay central.
...
**Problem F1:**
A dataset appears suitable for Parallel Coordinates. Decide what marks and channels would make the reader task explicit, and name one implementation requirement for the D3 version.
*Chapter this draws from: Chapter 50 — Parallel Coordinates*

**Problem F2:**
A second dataset looks more like OHLC Chart. Decide whether the previous chapter's method or the present chapter's method better matches the reader task.
*Chapter this draws from: Chapter 49 — OHLC Chart*

**Problem F3:**
A polished chart appears to use the right visual family, but the decisive issue is whether the encoded data structure matches Parallel Coordinates or a different chart logic from an earlier chapter. Choose the governing concept and justify it before solving.
...
- The student cannot explain how What this chart is connects to the chart choice.
